...
def update_test(network=None):
    ...
    for key in unique_country_keys:
        ...
        update_to = {"review": f"An updated review for {key}'s coffee."}
        ...
        if update_response and "hops" in update_response:
            ...
            total_updates += 1
            print(
                f"Updated key: {key}, Hops: {update_response['hops']}, Node: {random_node.node_id}"
            )
        else:
    ...
    if total_updates > 0:
        ...
        print(f"\nTotal Keys Updated: {total_updates}")
